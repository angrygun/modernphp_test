...
/*
 * PHP-FPM（PHP FastCGI Process Manager的简称，意思是“PHP FastCGI进程管理器”）是用于管理PHP进程池的软件，用于接收和处理来自Web服务器（例如nginx）的请求。PHP-FPM软件会创建一个主进程（通常以操作系统中根用户的身份运行），控制何时以及如何把HTTP请求转发给一个或多个子进程处理。PHP-FPM主进程还控制什么时候创建（处理Web应用更多的流量）和销毁（子进程运行时间太久或不再需要了）PHP子进程。PHP-FPM进程池中的每个进程存在的时间都比单个HTTP请求长，可以处理10、50、100、500或者更多的HTTP请求。
 *
 *
 *全局配置
 *
 ...
 *          PHP-FPM进程池监听的IP地址和端口号，让PHP-FPM只接受nginx从这里传入的请求，127.0.0.1:9000让指定的PHP-FPM进程池监听从本地端口9000进入的连接。我使用的端口是9000，不过你可以使用任何不需要特殊权限（大于1024）且没被其他系统进程占用的端口号。
